...
Provides counters, histograms, and gauges for tracking batch job lifecycle events,
item processing status, and performance metrics.
"""
...
batch_items_total = Counter(
    'batch_items_total', 
    'Total number of batch items by status',
    ['provider', 'status']
)
...
def record_item_status_change(provider: str, old_status: Optional[str], new_status: str, count: int = 1) -> None:
...
def record_job_duration(provider: str, status: str, duration_seconds: float) -> None:
    ...
    """
    batch_job_duration_seconds.labels(provider=provider, status=status).observe(duration_seconds)


def record_provider_latency(provider: str, operation: str, latency_seconds: float) -> None:
    ...
    """
    batch_provider_latency_seconds.labels(provider=provider, operation=operation).observe(latency_seconds)
