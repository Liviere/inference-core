...
try:
    ...
    raise ImportError(
        "Google GenAI package is required for Gemini batch provider. "
...
class GeminiBatchProvider(BaseBatchProvider):
    ...
    def __init__(self, config: Optional[dict] = None):
        ...
        if not api_key:
            raise ProviderPermanentError(
                "Google GenAI API key is required",
                self.PROVIDER_NAME
            )
        
        try:
            self.client = genai.Client(api_key=api_key)
            logger.debug(f"Initialized Gemini batch provider with config keys: {list(self.config.keys())}")
    ...
    def supports_model(self, model: str, mode: str) -> bool:
        ...
        """
        # Gemini batch API supports chat mode primarily
        supported_modes = ["chat"]
        
        # Common Gemini model patterns
        ...
        # Check if model matches Gemini patterns
        model_lower = model.lower()
        return any(pattern in model_lower for pattern in gemini_model_patterns)
    ...
    def prepare_payloads(self, batch_items: List[dict], model: str, mode: str, config: Optional[dict] = None) -> PreparedSubmission:
        ...
        try:
            for item in batch_items:
                ...
                gemini_request = self._convert_to_gemini_format(batch_item_id, input_payload, model, mode)
        ...
        logger.debug(f"Prepared {len(formatted_items)} items for Gemini batch submission")
    ...
    def _convert_to_gemini_format(self, custom_id: str, input_payload: dict, model: str, mode: str) -> dict:
        """
    ...
    def submit(self, prepared_submission: PreparedSubmission) -> ProviderSubmitResult:
        ...
        try:
            ...
            logger.info(f"Successfully submitted batch {batch_job.name} to Gemini with {len(submission_items)} items")
            ...
            raise ProviderPermanentError(
                f"Gemini batch submission failed: {str(e)}",
                self.PROVIDER_NAME
            )
    ...
    def poll_status(self, provider_batch_id: str) -> ProviderStatus:
        ...
        try:
            ...
            normalized_status = self.STATUS_MAPPING.get(batch_job.state, batch_job.state)
            
            # Build progress information
            progress_info = {
                "state": batch_job.state,
                "create_time": batch_job.create_time.isoformat() if batch_job.create_time else None,
                "update_time": batch_job.update_time.isoformat() if batch_job.update_time else None
            }
            ...
            logger.debug(f"Polled batch {provider_batch_id}: state={batch_job.state}, normalized={normalized_status}")
            ...
            raise ProviderTransientError(
                f"Gemini batch status polling failed: {str(e)}",
                self.PROVIDER_NAME
            )
        ...
        try:
            ...
            if hasattr(batch_job, 'dest') and batch_job.dest:
                ...
                if hasattr(batch_job.dest, 'inlined_responses') and batch_job.dest.inlined_responses:
                    results.extend(self._parse_inlined_responses(batch_job.dest.inlined_responses))
                else:
                    # Could be file-based results, but for now we focus on inlined
                    logger.warning(f"Batch {provider_batch_id} has non-inlined results which are not yet supported")
            
            logger.info(f"Fetched {len(results)} results from Gemini batch {provider_batch_id}")
            return results
            ...
            raise ProviderPermanentError(
                f"Gemini batch result fetching failed: {str(e)}",
                self.PROVIDER_NAME
            )
    
    def _parse_inlined_responses(self, inlined_responses: List[dict]) -> List[ProviderResultRow]:
        """
        ...
        for idx, response in enumerate(inlined_responses):
            try:
    ...
    def cancel(self, provider_batch_id: str) -> bool:
        ...
        try:
            ...
            if hasattr(result, 'state') and result.state in ["JOB_STATE_CANCELLED", "JOB_STATE_CANCELLING"]:
                logger.info(f"Successfully cancelled Gemini batch {provider_batch_id}")
                return True
            else:
                logger.warning(f"Gemini batch {provider_batch_id} cancellation returned unexpected state")
                return False
            ...
            raise ProviderPermanentError(
                f"Gemini batch cancellation failed: {str(e)}",
                self.PROVIDER_NAME
            )
