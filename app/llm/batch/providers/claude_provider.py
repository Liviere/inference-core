...
try:
    ...
    raise ImportError(
        "Anthropic package is required for Claude batch provider. "
...
class ClaudeBatchProvider(BaseBatchProvider):
    ...
    def __init__(self, config: Optional[dict] = None):
        ...
        try:
            ...
            logger.debug(f"Initialized Claude batch provider with config keys: {list(self.config.keys())}")
    ...
    def prepare_payloads(self, batch_items: List[dict], model: str, mode: str, config: Optional[dict] = None) -> PreparedSubmission:
        ...
        try:
            for item in batch_items:
                ...
                claude_request = self._convert_to_claude_format(batch_item_id, input_payload, model, mode)
        ...
        logger.debug(f"Prepared {len(formatted_items)} items for Claude batch submission")
    ...
    def _convert_to_claude_format(self, custom_id: str, input_payload: dict, model: str, mode: str) -> dict:
        """
    ...
    def submit(self, prepared_submission: PreparedSubmission) -> ProviderSubmitResult:
        ...
        try:
            ...
            logger.info(f"Successfully submitted batch {batch.id} to Claude with {len(prepared_submission.items)} items")
            ...
            submission_metadata = {
                ...
                "request_counts": {
                    "total": len(prepared_submission.items)
                }
            }
            ...
            raise ProviderPermanentError(
                f"Claude batch submission failed: {str(e)}",
                self.PROVIDER_NAME
            )
    ...
    def poll_status(self, provider_batch_id: str) -> ProviderStatus:
        ...
        try:
            ...
            if not batch:
                raise ProviderPermanentError(
                    f"Claude batch {provider_batch_id} not found",
                    self.PROVIDER_NAME
                )
            
            # Map Claude status to internal status
            normalized_status = self.STATUS_MAPPING.get(batch.processing_status, batch.processing_status)
            ...
            logger.debug(f"Polled batch {provider_batch_id}: status={batch.processing_status}, normalized={normalized_status}")
            ...
            raise ProviderTransientError(
                f"Claude batch status polling failed: {str(e)}",
                self.PROVIDER_NAME
            )
    ...
    def fetch_results(self, provider_batch_id: str) -> List[ProviderResultRow]:
        ...
        try:
            ...
            # Fetch results using the results API
            ...
            for entry in result_stream:
                try:
                    ...
                    if entry.result.type == "succeeded":
                        ...
                        if message.content:
                            for content_block in message.content:
                                if hasattr(content_block, 'text'):
                                    ...
                                    output_text = getattr(content_block, 'text', None)
                                    break
                        ...
                        is_success = False
                        error_message = f"Unknown result type: {entry.result.type}"
                    
                    results.append(ProviderResultRow(
                        custom_id=custom_id,
                        output_text=output_text,
                        output_data=output_data,
                        raw_metadata=entry.__dict__,
                    ...
                    # Create error result for unparseable entry
                    results.append(ProviderResultRow(
                        custom_id=getattr(entry, 'custom_id', 'unknown'),
                        output_text=None,
                        output_data=None,
                        raw_metadata=entry.__dict__ if hasattr(entry, '__dict__') else {},
                        error_message=f"Failed to parse result: {str(e)}",
                        is_success=False
                    ))
            
            logger.info(f"Fetched {len(results)} results from Claude batch {provider_batch_id}")
            return results
            ...
            raise ProviderPermanentError(
                f"Claude batch result fetching failed: {str(e)}",
                self.PROVIDER_NAME
            )
    ...
    def cancel(self, provider_batch_id: str) -> bool:
        ...
        try:
            ...
            if hasattr(self.client.messages.batches, 'cancel'):
                ...
                logger.warning(f"Claude batch cancellation not supported by current API version")
                return False
            ...
            raise ProviderPermanentError(
                f"Claude batch cancellation failed: {str(e)}",
                self.PROVIDER_NAME
            )
