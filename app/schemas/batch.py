...
from app.database.sql.models.batch import BatchEventType, BatchItemStatus, BatchJobStatus
...
class BatchJobBase(BaseModel):
    ...
    config_json: Optional[Dict[str, Any]] = Field(None, description="Batch configuration as JSON")
...
class BatchJobUpdate(BaseModel):
    ...
    provider_batch_id: Optional[str] = Field(None, description="External provider batch ID", max_length=255)
    submitted_at: Optional[datetime] = Field(None, description="When batch was submitted")
    completed_at: Optional[datetime] = Field(None, description="When batch completed")
    success_count: Optional[int] = Field(None, description="Number of successful requests", ge=0)
    error_count: Optional[int] = Field(None, description="Number of failed requests", ge=0)
    error_summary: Optional[str] = Field(None, description="Summary of errors")
    result_uri: Optional[str] = Field(None, description="Result download URI", max_length=500)
    config_json: Optional[Dict[str, Any]] = Field(None, description="Updated batch configuration")
...
class BatchJobResponse(BatchJobBase):
    ...
    provider_batch_id: Optional[str] = Field(None, description="External provider batch ID")
    submitted_at: Optional[datetime] = Field(None, description="When batch was submitted")
    completed_at: Optional[datetime] = Field(None, description="When batch completed")
...
class BatchItemBase(BaseModel):
    ...
    custom_external_id: Optional[str] = Field(None, description="Optional external identifier", max_length=255)
    input_payload: Optional[Dict[str, Any]] = Field(None, description="Request data as JSON")
...
class BatchItemUpdate(BaseModel):
    ...
    output_payload: Optional[Dict[str, Any]] = Field(None, description="Response data as JSON")
    error_detail: Optional[str] = Field(None, description="Error details if processing failed")
...
class BatchEventBase(BaseModel):
    ...
    old_status: Optional[str] = Field(None, description="Previous status", max_length=20)
...
class BatchJobWithItems(BatchJobResponse):
    ...
    items: List[BatchItemResponse] = Field(default_factory=list, description="Batch items")
...
class BatchJobStats(BaseModel):
    ...
    jobs_by_provider: Dict[str, int] = Field(..., description="Jobs grouped by provider")
    total_requests: int = Field(..., description="Total number of requests across all jobs")
    total_successes: int = Field(..., description="Total successful requests")
    total_errors: int = Field(..., description="Total failed requests")
    average_success_rate: float = Field(..., description="Average success rate across all jobs")
