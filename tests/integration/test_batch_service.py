...
class TestBatchServiceCRUD:
    ...
    async def test_query_batch_jobs(self, async_session_with_engine):
        ...
        jobs_data = [
            BatchJobCreate(provider="openai", model="gpt-4", mode="chat", request_count=10),
            BatchJobCreate(provider="anthropic", model="claude-3", mode="completion", request_count=5),
            BatchJobCreate(provider="openai", model="gpt-3.5", mode="chat", request_count=8),
        ]
    ...
    async def test_get_pending_jobs(self, async_session_with_engine):
        ...
        job1_data = BatchJobCreate(provider="openai", model="gpt-4", mode="chat", request_count=10)
        job1 = await service.create_batch_job(job1_data)
        
        job2_data = BatchJobCreate(provider="openai", model="gpt-4", mode="chat", request_count=5)
        job2 = await service.create_batch_job(job2_data)
        
        # Update one to completed
        await service.update_batch_job(job2.id, BatchJobUpdate(status=BatchJobStatus.COMPLETED))
...
class TestBatchItemOperations:
    ...
    async def test_get_batch_items(self, async_session_with_engine):
        ...
        job_data = BatchJobCreate(provider="openai", model="gpt-4", mode="chat", request_count=0)
        job = await service.create_batch_job(job_data)
        ...
        # Get items by status
        queued_items = await service.get_batch_items(job.id, status=BatchItemStatus.QUEUED)
    ...
    async def test_update_batch_item_and_job_counts(self, async_session_with_engine):
        ...
        item_update = BatchItemUpdate(
            ...
            output_payload={"response": "Success"}
        )
...
class TestBatchEventOperations:
    ...
    async def test_create_batch_event(self, async_session_with_engine):
        ...
        # Create job
        job_data = BatchJobCreate(provider="openai", model="gpt-4", mode="chat", request_count=10)
        job = await service.create_batch_job(job_data)
    ...
    async def test_get_batch_events(self, async_session_with_engine):
        ...
        events_data = [
            BatchEventCreate(event_type=BatchEventType.PROGRESS_UPDATE, event_data={"progress": 25}),
            BatchEventCreate(event_type=BatchEventType.ERROR, event_data={"error": "Network timeout"}),
            BatchEventCreate(event_type=BatchEventType.PROGRESS_UPDATE, event_data={"progress": 75}),
        ]
        ...
        progress_events = await service.get_batch_events(job.id, event_type=BatchEventType.PROGRESS_UPDATE)
        assert len(progress_events) == 2
        assert all(event.event_type == BatchEventType.PROGRESS_UPDATE for event in progress_events)
...
class TestBatchServiceStatistics:
    ...
    async def test_get_batch_stats(self, async_session_with_engine):
        ...
        await service.update_batch_job(
            jobs[0].id, 
            BatchJobUpdate(success_count=8, error_count=2, status=BatchJobStatus.COMPLETED)
        )
        await service.update_batch_job(
            jobs[1].id,
            BatchJobUpdate(success_count=5, error_count=0, status=BatchJobStatus.COMPLETED)
        )
...
class TestBatchServiceWithItems:
    ...
    async def test_bulk_update_items_status(self, async_session_with_engine):
        ...
        updated_count = await service.update_batch_items_status(job.id, status_updates)
