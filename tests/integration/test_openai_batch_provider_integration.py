...
from openai.types import Batch, FileObject, BatchRequestCounts
...
class TestOpenAIBatchProviderIntegration:
    ...
    @patch('app.llm.batch.providers.openai_provider.OpenAI')
    def test_end_to_end_successful_batch_flow(self, mock_openai_class):
        ...
        batch_items = [
            ...
            },
            {
                "id": "request-4",
                "input_payload": {
                    "messages": [{"role": "user", "content": "List the primary colors"}],
                    "max_tokens": 30
                }
            }
        ]
        ...
        mock_batch_validating.expires_at = int((datetime.now() + timedelta(hours=23)).timestamp())
        ...
        mock_batch_progress.expires_at = int((datetime.now() + timedelta(hours=22)).timestamp())
...
{"custom_id": "request-4", "response": {"body": {"choices": [{"message": {"content": "The primary colors are red, blue, and yellow."}}]}}}'''
        ...
        assert result_by_id["request-1"].output_text == "The capital of France is Paris."
        assert result_by_id["request-2"].output_text.startswith("Quantum computing uses")
...
{"custom_id": "success-2", "response": {"body": {"choices": [{"message": {"content": "I'm here to assist you."}}]}}}'''
        ...
        mock_client.files.content.side_effect = [mock_output_response, mock_error_response]
        ...
        assert failed_result.error_message == "[content_policy_violation] Request failed due to policy violation"
    ...
    def test_error_handling_and_status_transitions(self, mock_openai_class):
        ...
        batch_items = [
            {"id": "test-1", "input_payload": {"messages": [{"role": "user", "content": "Test"}]}}
        ]
        ...
        for provider_status, expected_normalized in status_sequence:
            ...
            mock_batch_status.request_counts.completed = 0 if provider_status != "completed" else 1
            mock_batch_status.request_counts.failed = 1 if provider_status in ["failed", "expired"] else 0
            mock_batch_status.completed_at = int(datetime.now().timestamp()) if provider_status in ["completed", "failed", "expired", "cancelled"] else None
