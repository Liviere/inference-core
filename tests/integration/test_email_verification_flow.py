...
@pytest.mark.asyncio
async def test_register_with_active_false_setting(async_test_client):
    """Test registration with AUTH_REGISTER_DEFAULT_ACTIVE=false"""
    with patch.dict(os.environ, {"AUTH_REGISTER_DEFAULT_ACTIVE": "false"}):
...
@pytest.mark.asyncio
async def test_login_denied_for_inactive_user(async_test_client):
...
@pytest.mark.asyncio
async def test_login_verified_required_setting(async_test_client):
    """Test login requires verified user when AUTH_LOGIN_REQUIRE_VERIFIED=true"""
    with patch.dict(os.environ, {"AUTH_LOGIN_REQUIRE_VERIFIED": "true"}):
        # Clear settings cache to pick up new env var
        from inference_core.core.config import get_settings
        get_settings.cache_clear()
...
@pytest.mark.asyncio
async def test_verified_user_can_login_when_required(async_test_client):
...
@pytest.mark.asyncio  
async def test_register_sends_verification_email_when_enabled(async_test_client):
    """Test registration sends verification email when AUTH_SEND_VERIFICATION_EMAIL_ON_REGISTER=true"""
    with patch.dict(os.environ, {"AUTH_SEND_VERIFICATION_EMAIL_ON_REGISTER": "true"}):
