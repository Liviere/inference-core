...
from app.celery.tasks.batch_tasks import batch_submit, batch_poll, batch_fetch, batch_retry_failed
...
class TestBatchSubmitTask:
    ...
    @patch('app.celery.tasks.batch_tasks.get_async_session')
    @patch('app.celery.tasks.batch_tasks.registry')
    def test_batch_submit_success(self, mock_registry, mock_get_session):
        ...
        with patch('app.celery.tasks.batch_tasks.BatchService', return_value=mock_batch_service):
            mock_job = MagicMock()
            ...
            mock_batch_service.get_batch_items.return_value = [
                MagicMock(id=uuid.uuid4(), sequence_index=0, input_payload={"test": "data"})
            ]
...
class TestBatchPollTask:
    ...
    @patch('app.celery.tasks.batch_tasks.get_sync_redis')
    @patch('app.celery.tasks.batch_tasks.get_async_session')
    @patch('app.celery.tasks.batch_tasks.select')
...
class TestBatchFetchTask:
    ...
    def test_batch_fetch_success(self, mock_registry, mock_get_session):
        ...
        with patch('app.celery.tasks.batch_tasks.BatchService', return_value=mock_batch_service):
            ...
            mock_items = [
                MagicMock(id=uuid.uuid4(), status=BatchItemStatus.QUEUED)
            ]
...
class TestBatchRetryFailedTask:
    ...
    @patch('app.celery.tasks.batch_tasks.get_async_session')
    @patch('app.celery.tasks.batch_tasks.batch_submit')
