...
from app.llm.batch.exceptions import ProviderRegistrationError
...
class MockBatchProvider(BaseBatchProvider):
    ...
    def submit(self, prepared_submission: PreparedSubmission) -> ProviderSubmitResult:
        ...
        if prepared_submission.model == "rate-limited-model":
            raise ProviderTransientError("Rate limit exceeded", self.PROVIDER_NAME, retry_after=60)
    ...
    def poll_status(self, provider_batch_id: str) -> ProviderStatus:
        ...
        if provider_batch_id not in self.submitted_batches:
            raise ProviderPermanentError(f"Batch {provider_batch_id} not found", self.PROVIDER_NAME)
        ...
        return ProviderStatus(
            ...
            progress_info={"completed": 0, "total": len(self.submitted_batches[provider_batch_id].items)}
        )
...
class TestBaseBatchProvider:
    ...
    def test_submit_permanent_error(self):
        ...
        batch_items = [{"input_payload": {"messages": [{"role": "user", "content": "Test"}]}}]
        prepared = self.provider.prepare_payloads(batch_items, "unsupported-model", "chat")
    ...
    def test_submit_transient_error(self):
        ...
        batch_items = [{"input_payload": {"messages": [{"role": "user", "content": "Test"}]}}]
        prepared = self.provider.prepare_payloads(batch_items, "rate-limited-model", "chat")
...
class TestBatchProviderRegistry:
    ...
    def test_register_invalid_provider_class(self):
        ...
        with pytest.raises(ProviderRegistrationError, match="must inherit from BaseBatchProvider"):
