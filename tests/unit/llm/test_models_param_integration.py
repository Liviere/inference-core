...
class TestLLMModelFactoryParameterNormalization:
    ...
    def test_openai_model_uses_normalized_params(self, mock_chat_openai, mock_normalize):
        ...
        mock_normalize.assert_called_once_with(ModelProvider.OPENAI, expected_raw_params)
    ...
    def test_gemini_model_uses_normalized_params(self, mock_chat_gemini, mock_normalize):
        ...
        mock_normalize.assert_called_once_with(ModelProvider.GEMINI, expected_raw_params)
        ...
        assert "max_tokens" not in call_args.kwargs  # Should be renamed to max_output_tokens
        
        assert result == mock_model

    @patch('app.llm.models.normalize_params')
    @patch('app.llm.models.ChatAnthropic')
    def test_claude_model_uses_normalized_params(self, mock_chat_anthropic, mock_normalize):
        ...
        mock_normalize.assert_called_once_with(ModelProvider.CLAUDE, expected_raw_params)
    ...
    @patch('app.llm.models.normalize_params')
    def test_unsupported_provider_error(self, mock_normalize):
        ...
        # Mock normalize_params to return empty dict (doesn't matter for this test)
        mock_normalize.return_value = {}
        
        # Execute
        with patch('app.llm.models.logger') as mock_logger:
            result = self.factory._create_model_instance(config)
...
class TestProviderSpecificParameterHandling:
    ...
    @patch('app.llm.models.ChatAnthropic')
    @patch('app.llm.models.normalize_params')
    def test_claude_no_manual_parameter_filtering(self, mock_normalize, mock_chat_anthropic):
