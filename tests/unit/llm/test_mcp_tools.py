...
import inference_core.llm.mcp_tools
...
class TestMCPToolManager:
    ...
    def reset_mcp_manager(self):
        """Reset global MCP manager before and after each test"""
        inference_core.llm.mcp_tools._mcp_tool_manager = None
        yield
        inference_core.llm.mcp_tools._mcp_tool_manager = None

    @pytest.fixture(autouse=True)
    def mock_global_llm_config(self):
        """Mock global LLM config to avoid reading from file"""
        with patch("inference_core.llm.mcp_tools.get_llm_config") as mock_get:
            mock_config = MagicMock()
            # Default MCP config (disabled)
            mock_config.mcp_config = MCPConfig()
    ...
    def test_init_with_custom_config(self):
        ...
        custom_config = MCPConfig(enabled=True, require_superuser=False)
    ...
    async def test_get_tools_no_profile_no_default(self):
        ...
        config = MCPConfig(enabled=True, require_superuser=False, default_profile=None)
        manager = MCPToolManager(mcp_config=config)
    ...
    async def test_get_tools_profile_not_found(self):
        ...
        config = MCPConfig(enabled=True, require_superuser=False, profiles={})
        manager = MCPToolManager(mcp_config=config)
