...
class TestNormalizeParams:
    ...
    def test_gemini_params_normalized(self, mock_logger):
        ...
        mock_logger.debug.assert_any_call(
            "Parameter renamed for gemini: max_tokens -> max_output_tokens"
        )
        
        # Should log dropped parameters
        mock_logger.debug.assert_any_call(
            "Parameter dropped for gemini: frequency_penalty (value: 0.1)"
        )
        mock_logger.debug.assert_any_call(
            "Parameter dropped for gemini: presence_penalty (value: 0.2)"
        )
        mock_logger.debug.assert_any_call(
            "Parameter dropped for gemini: request_timeout (value: 30)"
        )
    ...
    def test_claude_params_normalized(self, mock_logger):
        ...
        # Should log parameter rename
        mock_logger.debug.assert_any_call(
            "Parameter renamed for claude: request_timeout -> timeout"
        )
        
        # Should log dropped parameters
        mock_logger.debug.assert_any_call(
            "Parameter dropped for claude: frequency_penalty (value: 0.1)"
        )
        mock_logger.debug.assert_any_call(
            "Parameter dropped for claude: presence_penalty (value: 0.2)"
        )
    ...
    def test_unknown_parameter_warning(self, mock_logger):
        ...
        mock_logger.warning.assert_called_once_with(
            "Unknown parameter for openai: unknown_param (value: value) - dropping"
        )
