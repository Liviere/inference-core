...
from inference_core.observability.logging import BatchLogger, get_batch_logger
...
from inference_core.observability.sentry import BatchSentryIntegration
...
class TestBatchMetrics:
    ...
    def test_record_job_status_change(self):
        ...
        batch_jobs_key = next(
            (k for k in summary.keys() if k.startswith("batch_jobs")), None
        )
    ...
    def test_record_item_status_change(self):
        ...
        batch_items_key = next(
            (k for k in summary.keys() if k.startswith("batch_items")), None
        )
        ...
        completed_sample = next(
            s for s in test_samples if s["labels"]["status"] == "completed"
        )
        assert completed_sample["value"] == 5.0

        failed_sample = next(
            s for s in test_samples if s["labels"]["status"] == "failed"
        )
    ...
    def test_record_provider_latency(self):
        ...
        test_samples = [
            s for s in latency_samples if s["labels"]["provider"] == provider
        ]
    ...
    def test_record_job_duration(self):
        ...
        test_samples = [
            s for s in duration_samples if s["labels"]["provider"] == provider
        ]
        assert len(test_samples) > 0
    ...
    def test_update_jobs_in_progress(self):
        ...
        test_samples = [
            s for s in progress_samples if s["labels"]["provider"] == provider
        ]
    ...
    def test_time_provider_operation_decorator(self):
        ...
        from inference_core.observability.metrics import time_provider_operation
    ...
    def test_record_error_and_retry(self):
        ...
        batch_errors_key = next(
            (k for k in summary.keys() if k.startswith("batch_errors")), None
        )
        batch_retry_key = next(
            (k for k in summary.keys() if k.startswith("batch_retry")), None
        )
        ...
        test_error_samples = [
            s for s in error_samples if s["labels"]["provider"] == provider
        ]
        test_retry_samples = [
            s for s in retry_samples if s["labels"]["provider"] == provider
        ]
...
class TestBatchLogger:
    ...
    @patch("inference_core.observability.logging.BatchLogger._build_batch_fields")
    def test_info_logging(self, mock_build_fields):
        ...
        with patch.object(self.logger, "logger") as mock_logger:
            self.logger.info(
                "Test message", job_id="test-job", provider="openai", operation="test"
            )

            mock_logger.info.assert_called_once_with(
                "Test message", extra={"test": "fields"}
            )
    ...
    def test_debug_rate_limiting(self):
        ...
        with patch.object(self.logger, "logger") as mock_logger:
            ...
            for i in range(15):
                self.logger.debug(
                    f"Debug message {i}", job_id="test-job", operation="rate_limit_test"
                )
...
class TestBatchSentryIntegration:
    ...
    @patch("inference_core.observability.sentry.SENTRY_AVAILABLE", False)
        ...
        with patch.object(self.sentry, "enabled", True):
            with patch.object(self.sentry, "set_batch_context"):
                event_id = self.sentry.capture_batch_error(
                    error, job_id=job_id, provider="openai", operation="test"
                )
    ...
    def test_log_status_change(self):
        ...
        with patch.object(self.sentry, "add_breadcrumb") as mock_add_breadcrumb:
            with patch.object(self.sentry, "enabled", True):
                ...
                assert (
                    call_args.kwargs["data"]["status_transition"]
                    == "created -> submitted"
                )
