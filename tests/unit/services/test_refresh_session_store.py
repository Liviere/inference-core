"""
Unit tests for inference_core.services.refresh_session_store module
...
class TestRefreshSessionStore:
    ...
    @patch("inference_core.services.refresh_session_store.get_redis")
    @patch("inference_core.services.refresh_session_store.get_settings")
    ...
    def test_key_method(self):
        ...
        with patch(
            "inference_core.services.refresh_session_store.get_settings"
        ) as mock_get_settings:
            mock_settings = MagicMock()
            mock_settings.redis_refresh_prefix = "auth:refresh:"
            mock_get_settings.return_value = mock_settings

            with patch("inference_core.services.refresh_session_store.get_redis"):
                store = RefreshSessionStore()
    ...
    async def test_add_success(self, mock_get_settings, mock_get_redis):
        ...
        mock_redis.hset.assert_called_once_with(
            "auth:refresh:test-jti", mapping={"sub": "user123", "exp": future_exp}
        )
    ...
    @patch("inference_core.services.refresh_session_store.get_settings")
    @pytest.mark.asyncio
    async def test_decode_and_validate_refresh_success(
        self, mock_get_settings, mock_get_redis
    ):
    ...
    @patch("inference_core.services.refresh_session_store.get_settings")
    @pytest.mark.asyncio
    async def test_decode_and_validate_refresh_wrong_type(
        self, mock_get_settings, mock_get_redis
    ):
    ...
    @patch("inference_core.services.refresh_session_store.get_settings")
    @pytest.mark.asyncio
    async def test_decode_and_validate_refresh_missing_fields(
        self, mock_get_settings, mock_get_redis
    ):
    ...
    @patch("inference_core.services.refresh_session_store.get_settings")
    @pytest.mark.asyncio
    async def test_decode_and_validate_refresh_session_not_found(
        self, mock_get_settings, mock_get_redis
    ):
    ...
    @patch("inference_core.services.refresh_session_store.get_settings")
    @pytest.mark.asyncio
    async def test_decode_and_validate_refresh_invalid_token(
        self, mock_get_settings, mock_get_redis
    ):
