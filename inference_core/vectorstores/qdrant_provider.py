...
class QdrantProvider(BaseVectorStoreProvider):
    ...
    async def ensure_collection(self, name: str, dimension: int = None) -> bool:
        ...
        try:
            ...
            self.logger.info(f"Created Qdrant collection '{name}' with dimension {dimension}")
            return True
    ...
    async def add_texts(
        ...
        self.logger.info(f"Added {len(texts)} documents to Qdrant collection '{collection}'")
        return list(ids)
    ...
    async def similarity_search(
        ...
        **kwargs,
        ...
        self.logger.debug(f"Found {len(documents)} similar documents for query in collection '{collection}'")
        return documents
    ...
    async def collection_stats(self, collection: str) -> CollectionStats:
        ...
        try:
            ...
            if not vector_params:
                raise ValueError(f"No vector configuration found for collection '{collection}'")
            
            dimension = vector_params.size
            distance_metric = vector_params.distance.value
            ...
            raise ValueError(f"Collection '{collection}' does not exist or is not accessible") from e
    ...
    async def delete_collection(self, collection: str) -> bool:
        ...
        try:
            ...
            self.logger.warning(f"Collection '{collection}' does not exist or could not be deleted")
            return False
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "backend": "qdrant",
                "url": self.url,
                "cluster_status": cluster_info.status.value if hasattr(cluster_info, 'status') else "unknown",
