...
class VectorStoreService:
    ...
    def provider(self) -> Optional[BaseVectorStoreProvider]:
        """Get the vector store provider, initializing if needed"""
    ...
    async def get_collection_stats(self, collection: Optional[str] = None) -> CollectionStats:
        """
    ...
    async def delete_collection(self, collection: str) -> bool:
        ...
        cache_keys_to_remove = [key for key in self._retrievers_cache.keys() if key.startswith(f"{collection}:")]
        ...
        for key, value in metadata.items():
            ...
            if isinstance(value, (str, int, float, bool)):
                ...
                for nested_key, nested_value in value.items():
                    if isinstance(nested_key, str) and isinstance(nested_value, (str, int, float, bool)):
