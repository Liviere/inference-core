...
DEFAULT_LLM_MOCK_EMULATION_ENV = {
    "LLM_EMULATION_PROFILE": "performance-realistic",
...
def _build_locust_env(
    ...
    if profile_name == "llm_mock":
        ...
        for key, value in DEFAULT_LLM_MOCK_EMULATION_ENV.items():
...
def _print_run_summary(
    ...
    if profile_name == "llm_mock":
        ...
        print(
            "LLM latency profile: "
            f"base={env['LLM_EMULATION_LATENCY_MS']}ms, "
            f"jitter=+/-{env['LLM_EMULATION_LATENCY_JITTER_MS']}ms, "
            "session-scale="
            f"{env['LLM_EMULATION_SESSION_SCALE_MIN']}.."
            f"{env['LLM_EMULATION_SESSION_SCALE_MAX']}, "
            f"step-growth={env['LLM_EMULATION_STEP_LATENCY_GROWTH']}, "
            "first-chunk-ratio="
            f"{env['LLM_EMULATION_STREAM_FIRST_CHUNK_RATIO']}"
        )
